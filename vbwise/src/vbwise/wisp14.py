...
class App:
    ...
    def build(self):
        ...
        def configure_widgets(name, node):
            ...
            if "grid" in props:
                weight = props['grid'].get('weight', {})
                grid_props = props["grid"]
                try:
                    del grid_props['weight']
                except:
                    pass
                widget.grid(**grid_props)
                # Set grid weights for resizing if provided
                if "weight":
                    row = grid_props.get("row", 0)
                    column = grid_props.get("column", 0)
                    # weight = grid_props["weight"]
                    if isinstance(weight, dict):
                        if "row" in weight:
                            widget.grid_rowconfigure(row, weight=weight["row"])
                        if "column" in weight:
                            widget.grid_columnconfigure(column, weight=weight["column"])
...
# Define the layout with a scrollable frame
layout = {
    "root": {
        "type": "Tk",
        "props": {
        ...
        "parts": {
            ...
            "content_frame": {
                "type": "Frame",
                "props": {
                    # No grid here; placed inside canvas via create_window
                },
                "parts": {
                    # Content widgets can be added here later
                }
            }
        }
    }
